...
def set_seed(seed):
    ...
    ###############Parameters########################
noise_flag = 1 ## 0 =no noise
noise_std = 1 ####3.0
add_prior_flag = True
num_minimize_init = 32
qual_index = [7]
...
number_of_artificial=3
new_artificial_0_level=[]
new_artificial_1_level=[]
new_artificial_2_level=[]
new_artificial_3_level=[]
new_artificial_4_level=[]

sampelnumber__0_level=[]
sampelnumber__1_level=[]
sampelnumber__2_level=[]
sampelnumber__3_level=[]
sampelnumber__4_level=[]

bigest_actual_level=int(torch.max(train_x[:,-1]))
h=1
train_xx= train_x.clone()
#while np.size(new_artificial_0_level,0) < number_of_artificial and new_artificial_1_level < number_of_artificial and new_artificial_2_level < number_of_artificial and new_artificial_3_level < number_of_artificial and new_artificial_4_level < number_of_artificial:
for i in range(0,num_samples_train):
    if train_xx[i, -1]==0 and np.size(new_artificial_0_level,0) < number_of_artificial:
        #new_artificial_0_level.append(int(np.array(train_xx[i, -1]))+4+h)
        new_artificial_0_level.append(bigest_actual_level+h)
        sampelnumber__0_level.append(i)
        train_xx[i, -1]=bigest_actual_level+h
        h+=1
    elif train_xx[i, -1]==1 and np.size(new_artificial_1_level,0) < number_of_artificial:
        new_artificial_1_level.append(bigest_actual_level+h)
        train_xx[i, -1]=bigest_actual_level+h 
        h+=1  
        sampelnumber__1_level.append(i)
    elif train_xx[i, -1]==2 and np.size(new_artificial_2_level,0) < number_of_artificial:
        new_artificial_2_level.append(bigest_actual_level+h) 
        train_xx[i, -1]=bigest_actual_level+h
        h+=1
        sampelnumber__2_level.append(i)
    elif train_xx[i, -1]==3 and np.size(new_artificial_3_level,0) < number_of_artificial:
        new_artificial_3_level.append(bigest_actual_level+h) 
        train_xx[i, -1]=bigest_actual_level+h
        h+=1
        sampelnumber__3_level.append(i)
    elif train_xx[i, -1]==4 and np.size(new_artificial_4_level,0) < number_of_artificial:
        new_artificial_4_level.append(bigest_actual_level+h)
        train_xx[i, -1]=bigest_actual_level+h
        h+=1
        sampelnumber__4_level.append(i)
######################################################################################
# start timing
...
set_seed(123)
level_sets =[int(torch.max(train_xx[:, -1]))+1]#[len(NAN_Lis)+5]# [4]
model2 = LMGP(
    train_x=train_xx,
    train_y=train_y,
    ...
    NN_layers= [],
    fix_noise= False,# For continuation should be True
    lb_noise = 1e-20
).double()
...
#fit_model_scipy# For continuation should noise_tune2
...
print('######################################')
######################################################
X_TRAIN_KNN=np.concatenate((positions[0:5,:].data, positions[new_artificial_0_level,:],positions[new_artificial_1_level,:],positions[new_artificial_2_level,:],positions[new_artificial_3_level,:],positions[new_artificial_4_level,:]), axis=0)
Y_TRAIN_KNN=np.concatenate((0,1,2,3,4, 0*(np.ones(np.size(new_artificial_0_level))),1*(np.ones(np.size(new_artificial_1_level))),2*(np.ones(np.size(new_artificial_2_level))),3*(np.ones(np.size(new_artificial_3_level))),4*(np.ones(np.size(new_artificial_4_level)))), axis=None)
np.concatenate((0,1,2,3,4, 0*(np.ones(np.size(new_artificial_0_level))),1*(np.ones(np.size(new_artificial_1_level))),2*(np.ones(np.size(new_artificial_2_level))),3*(np.ones(np.size(new_artificial_3_level))),4*(np.ones(np.size(new_artificial_4_level)))), axis=None)

####################################################
#neigh_new = KNeighborsClassifier(n_neighbors=number_of_artificial+1)
neigh_new = KNeighborsClassifier(n_neighbors=1)
neigh_new.fit(X_TRAIN_KNN, Y_TRAIN_KNN)
predict_based_on_latent_map=neigh_new.predict(np.array(positions[5:5+int(Number_of_missing_data),:].data))
accuracy_final= np.sum(np.array(X_Y_test_lable) == predict_based_on_latent_map)/len(predict)
