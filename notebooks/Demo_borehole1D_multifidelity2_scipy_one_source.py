...
Optimization_technique='Newton-CG' #optimization methods: 'BFGS' 'L-BFGS-B' 'Newton-CG' 'trust-constr'
Optimization_constraint=False        # False
regularization_parameter=[10000, 0] ###### Always at least one element of regularization_parameter should be zero: The first element is for L1 regularization and the second element is for L2
##################################
...
train_x = torch.from_numpy(np.float64(train_x[:50,:-1]))
train_y = torch.from_numpy(np.float64(train_y[:50,:]))
test_x  = torch.from_numpy(np.float64(test_x[:10000,:-1]))
test_y  = torch.from_numpy(np.float64(test_y[:10000,:]))
